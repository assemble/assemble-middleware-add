...
var add = require('../')(assemble);


describe('assemble-middleware-add', function() {
  describe('add()', function() {
    it('should return a stream', function(done) {
      var stream = add(join(__dirname, "./fixtures/*.coffee"));
      should.exist(stream);
      should.exist(stream.on);
      done();
    });
    it('should return a input stream from a flat glob', function(done) {
      var stream = assemble.src('!*').pipe(add(join(__dirname, "./fixtures/*.coffee")));
    ...
    it('should return a input stream for multiple globs', function(done) {
      ...
      var stream = assemble.src('!*').pipe(add(globArray));
    ...
    it('should return a input stream with no contents when read is false', function(done) {
      var stream = assemble.src('!*').pipe(add(join(__dirname, "./fixtures/*.coffee"), {read: false}));
      stream.on('error', done);
      stream.on('data', function(file) {
        should.exist(file);
        should.exist(file.path);
    ...
    it('should return a input stream with contents as stream when buffer is false', function(done) {
      var stream = assemble.src('!*').pipe(add(join(__dirname, "./fixtures/*.coffee"), {buffer: false}));
      stream.on('error', done);
      stream.on('data', function(file) {
        should.exist(file);
        should.exist(file.path);
    ...
    it('should return a input stream from a deep glob', function(done) {
      var stream = assemble.src('!*').pipe(add(join(__dirname, "./fixtures/**/*.jade")));
      stream.on('error', done);
      stream.on('data', function(file) {
        should.exist(file);
        should.exist(file.path);
    ...
    it('should return a input stream from a deeper glob', function(done) {
      var stream = assemble.src('!*').pipe(add(join(__dirname, "./fixtures/**/*.dmc")));
      var a = 0;
      stream.on('error', done);
    ...
    it('should return a file stream from a flat path', function(done) {
      var a = 0;
      var stream = assemble.src('!*').pipe(add(join(__dirname, "./fixtures/test.coffee")));
      stream.on('error', done);
      stream.on('data', function(file) {
        ++a;
        should.exist(file);
